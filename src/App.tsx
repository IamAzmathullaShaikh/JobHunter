...
import CoverLetterEngine from "./components/CoverLetterEngine.tsx";
import PrepStudio from "./components/PrepStudio.tsx";
import {
  Sparkles,
  Briefcase,
  Brain,
...
function Dashboard() {
  ...
  const handleUpdateApplicationCard = async (appId: number, status: ApplicationStatus, notes: string) => {
    try {
      const response = await fetch("/api/tracker/update", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ application_id: appId, status, notes }),
      });
      if (response.ok) {
        setJobs((prev) =>
          prev.map((j) =>
            j.application?.id === appId
              ? { ...j, application: { ...j.application, status, notes } }
              : j
          )
        );
      ...
      <main className="flex-1 max-w-[1440px] w-full mx-auto p-6">
        {isLoading ? (
          ...
          <div className="space-y-6 animate-fade-in">
             ...
             {activeTab === "cover" && (
               <CoverLetterEngine resumeText={resumeText} />
             )}

             {activeTab === "prep" && (
               <PrepStudio resumeText={resumeText} />
             )}
