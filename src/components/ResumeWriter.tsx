        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
          ...
          <div className="space-y-4 flex flex-col">
             ...
             {optimizedBullets.length > 0 && (
               <button 
                 onClick={copyBullets}
                 className="bg-slate-800 hover:bg-slate-700 text-white font-bold py-3 rounded-2xl flex items-center justify-center gap-2 transition-all">
                  {copied ? <Check className="w-4 h-4 text-emerald-400" /> : <Copy className="w-4 h-4" />}
                  {copied ? "Copied to Clipboard!" : "Copy All Optimized Bullets"}
               </button>
             )}
          </div>
        </div>
